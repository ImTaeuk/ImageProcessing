...
void ApplyLPF()
{
    ...
    for (m = 0; m < N; m++)
    {
        f[m] = (unsigned char*)malloc(sizeof(unsigned char) * N);
    }
    ...
    for (m = 0; m < N; m++)
    {
        for (n = 0; n < N; n++)
        {
            ...
            double dist = sqrt(pow((double)m - 128.0, 2) + pow((double)n - 128, 2));
            double filterVal;
